...
export async function whenTheContractIsPaused() {
  ...
  it('Should revert', async () => {
    expect(
      goodKernelContract.pauseGoodService({
...
export async function whenTheConditionsAreMet() {
  ...
  it('Should set the GoodServiceState to Paused', async () => {
    await goodKernelContract.pauseGoodService({
      from: users.client.address,
      goodId: 1,
      goodServiceId: 1,
    ...
    // 0 = Paused
    expect(goodServiceState.toNumber()).to.be.equal(0);
  });

  it('Should emit a GoodServicePaused event', async () => {
    expect(
      goodKernelContract.pauseGoodService({
        from: users.client.address,
        goodId: 1,
    ...
    ).to.emit(goodKernelContract, 'GoodServicePaused');
  });
}
